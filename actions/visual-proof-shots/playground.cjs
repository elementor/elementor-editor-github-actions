...
async function waitForWpFrame(page) {
			...
			if (looksReady) {
				const hasBody = await frame
					.evaluate(() =>
						Boolean(
							document.body &&
							document.body.innerHTML.length > 20,
						),
					)
					.catch(() => false);
				if (hasBody) {
	...
	throw new Error(
		'Playground WordPress frame did not become ready after 45 attempts',
	);
}
...
async function addCaption(page, text) {
	...
	await page.evaluate((caption) => {
		...
		if (!bar) {
			...
			bar.style.cssText =
				'position:fixed;left:0;right:0;bottom:0;z-index:2147483647;padding:12px 16px;background:#1f2937;color:#fff;font:14px/1.4 sans-serif;white-space:pre-wrap;max-height:28%;overflow:hidden;';
