...
async function main() {
	...
	try {
		...
		const openedEditor =
			(await clickFirstInWp(page, 'link', 'Add New')) ||
			(await clickFirstInWp(page, 'link', 'Add New Page'));
		if (openedEditor) {
			await page.waitForTimeout(2000);
			(await clickFirstInWp(page, 'button', 'Edit with Elementor')) ||
				(await clickFirstInWp(page, 'link', 'Edit with Elementor'));
		...
		try {
			...
			console.error(
				`[visual-proof:capture] video save failed: ${videoError.message || videoError}`,
			);
		}
		await browser.close();
	}
	...
	if (0 === shots.length) {
		throw new Error(
			'No screenshots were captured (zero PNG files written)',
		);
	}
}
