...
describe('patchPhpVersion', () => {
	...
	it('updates both markers in a single call', () => {
		...
		expect(result).toContain(
			"define( 'ELEMENTOR_VERSION', '5.20.0-beta3' );",
		);
	});
...
describe('patchReadmeTxt', () => {
	...
	it('updates both stable and beta tags', () => {
		const result = patchReadmeTxt(original, {
			stable: '4.0.0',
			beta: '4.1.0-beta1',
		});
	...
	it('throws when "Stable tag:" line is missing from content', () => {
		...
		expect(() => {
			patchReadmeTxt(noStable, { stable: '4.0.0', beta: '4.1.0-beta1' });
	...
	it('throws when "Beta tag:" line is missing from content', () => {
		...
		expect(() => {
			patchReadmeTxt(noBeta, { stable: '4.0.0', beta: '4.1.0-beta1' });
...
describe('parseLatestTagFromLsRemote', () => {
	it('returns the latest stable tag', () => {
		expect(
			parseLatestTagFromLsRemote(LS_REMOTE_OUTPUT, STABLE_PATTERN),
		).toBe('4.0.0');
	});

	it('returns the latest beta tag', () => {
		expect(parseLatestTagFromLsRemote(LS_REMOTE_OUTPUT, BETA_PATTERN)).toBe(
			'4.1.0-beta1',
		);
	});
	...
	it('strips refs/tags/ prefix', () => {
		...
		expect(parseLatestTagFromLsRemote(output, STABLE_PATTERN)).toBe(
			'1.2.3',
		);
	});
